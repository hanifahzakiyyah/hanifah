    ...
    if (masuk[0] == "ل" && lepasHarokat[2] == " ") {
        ...
        // isim alat  مِفعل
            ...
            if(hurufmadhi.indexOf(lepasHarokat[3]) >= 0 && lepasHarokat.length == 4){   // huruf nya 4, dan ada tanda madhi
                ...
                if(lepasHarokat[lepasHarokat.length-1] == "ن" && lepasHarokat[lepasHarokat.length-2] == ("ا" || "و")) { // semua isim makan
                    katadasar = lepasHarokat.replace(/م/, "").slice(0, 3);
                    type = ["isim makan atau zaman", "isim alat sink iki po yo"];
                } else if(lepasHarokat[4] == "ة") {
                    katadasar = lepasHarokat.replace(/م/, "").slice(0, 3);
                    type = ["isim makan atau zaman", "isim alat dalam bentuk muannast"];
                } else {
                    katadasar = lepasHarokat.slice(0, 3);
                    type = "fiil madhi"
                }
        ...
        katadasar = lepasHarokat.slice(0, 2) + "ّ";
        type = "fiil madhi mudoaf masuk sini ada ga";
    } else { // madhi shohih
        katadasar = lepasHarokat.slice(0, 3);
        type = "fiil madhi masuk sini ada ga" 
    } 
